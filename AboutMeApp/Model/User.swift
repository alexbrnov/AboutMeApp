...
struct Person {
    ...
    static func getInfo() -> Person {
        Person(
            ...
            bio: "Типичный миллениал из СПБ). Работаю из дома и слежу за розовыми курьерами, иногда пишу треки. Воспитываем с женой собаку, живем обычным питерский лайфстайлом: фильтр-кофе, рамены, фо-бо, смузи сауры, булки из Вольчека(if you know you know) вот это вот все). К 28 годам становится невыносима местная зима. Мечтаю стать разработчиком, чтобы эвакуироваться в Тайланд на этот период)"
            )
    }
